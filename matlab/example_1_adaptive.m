...
close all
clc
global L d k1 k2 k3 ar v_dist w_dist
tic
...
d0=0;
d=[0;d0]';
p0=[-16 26 1*pi]';          % initial value for [x, y, w]
p00=[1 1 -2*pi]';                   % initial value for [x, y, w]
d_est_initial = [0.5 -0.25]';       % initial value for [v_hat, w_hat]

%% initial state
z0=[p0;p00;d_est_initial];

%% ode with fixed step
k=1e-2; tfinal=30;
options=odeset('reltol',1e-12,'abstol',1e-12);
[t,z]=ode45('f_track_1_adaptive',0:k:tfinal,z0,options); 

% ode ref
[t_ref,z_ref]=ode45('f_track_1_adaptive_ref',0:k:tfinal,z0,options); 
...
% adaptive trajectory
	...
	v = v0 * cos(ae) + k1 * xe             - v_hat;
	w = w0 + v0 * (k2 * ye + k3 * sin(ae)) - w_hat;
